...
public:
  explicit TableData(
      ...
      : tableId(tableId), decoderName(decoderName), json(json)
  {
  }
...
public:
  FeedWorker(Nan::Callback *callback,
      ...
      : AsyncFactoryWorker(callback, resource_name), m_progress(progress), m_buf(NULL), m_buf_len(0), m_ret(-1)
  {
    m_obj = Nan::ObjectWrap::Unwrap<dvbteeParser>(info.Holder());
  ...
  void HandleOKCallback()
  {
    ...
    v8::Local<v8::Value> argv[] = {
        Nan::Null(), Nan::New<v8::Number>(m_ret)};
